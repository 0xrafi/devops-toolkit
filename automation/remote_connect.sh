...
manjaro_alias="${MANJARO_SSH_ALIAS}"
manjaro_mac_address="${MAC_ADDRESS}" # local Mac address of Manjaro server
...
wake_up_server() {
  ...
  ssh "${jump_connection}" "sudo etherwake -i eth0 ${manjaro_mac_address}"
  echo "Waiting for server to wake up..."
  sleep $INITIAL_WAIT_TIME
  while true; do
    if ssh -q -o "ConnectTimeout=5" -o "StrictHostKeyChecking=no" "${manjaro_alias}" "exit" 2>/dev/null; then
      echo "Server is up!"
      break
    else
...
initiate_remote_shutdown() {
  ...
  ssh -A -t "${manjaro_alias}" "sudo shutdown now"
}
...
establish_manjaro_connection() {
  ...
  if is_yes_response "$tunnel"; then
    ssh -f -N -L $port:localhost:$port "${manjaro_alias}"
  else
